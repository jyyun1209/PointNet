...
class PointNet_wo_transform(nn.Module):
	def __init__(self, k=10):
		super(PointNet_wo_transform, self).__init__()
		self.name = 'PointNet_wo_transform'
	...
	def forward(self, x):								# input: [B, 3, N]
		...
		x = self.shared_mlp_2(x)				# after_mlp2: [B, 1024, N]

		x, _ = torch.max(x, 2, keepdim=True)
		x = torch.squeeze(x, 2)		# global_feature: [B, 1024]
		
		x = self.out_mlp(x)					# after_out_mlp: [B, 10]
